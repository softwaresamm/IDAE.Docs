...
| `File`                       | _(pendiente)_                       | Adjunto de archivo                                                            |
| `Signature`                  | _(pendiente)_                       | Firma digital                                                                 |
...
## Paso 6 — Guardado del Formulario

Al hacer submit, `buildFormInsertPayload(form, values)` (`lib/form-payload.ts`)
transforma los `FormValues` internos (claves `field_<id>` y `field_<id>__comment`) en el
DTO `FormInsertDto`:

```typescript
// FormValues internos: { field_42: '5', field_42__comment: 'nota', field_43: 'texto' }

interface FormInsertFieldDto {
  id: number;
  textValue: string;
  selectedValue: number[];
}
interface FormInsertDto {
  idForm: number;
  fields: FormInsertFieldDto[];
}
```

Mapeo por `FieldType`:

| Tipo                                      | `selectedValue`                      | `textValue`                        |
| ----------------------------------------- | ------------------------------------ | ---------------------------------- |
| `List` / `ConditionalList` (+`NoComment`) | ids seleccionados                    | comentario (`field_<id>__comment`) |
| `MultipleSelection` (+`NoComment`)        | ids seleccionados                    | comentario                         |
| `Boolean`                                 | `[]`                                 | `'true'` / `'false'`               |
| Texto, número, fecha, `ReadOnly`          | `[]`                                 | valor serializado a string         |
| `File` / `Signature`                      | _(se omiten — aún no implementados)_ | —                                  |

`idForm` es el id real del formulario, tomado del slug de la ruta (`parts.at(-2)`, p.ej.
`cargos-12-34` → `12`) — la respuesta de estructura del formulario no lo incluye.

El envío se hace vía el hook `useCreateFormRecord` (`hooks/features/forms/use-create-form-record.ts`),
que compone `useFormInsert(idFunctionality)` →
`POST ROUTES.CREATE_FORM_ENTITY` (`forms/insert`, enrutado por el gateway), con header
`IdFunctionality: <id>`. En éxito muestra el snackbar
`RECORD_CREATED` e invalida `['form-objects', idForm]`; en error usa `handleApiError`.

`idFunctionality` se extrae del slug de la ruta (`parts.at(-1)`), p.ej. `cargos-12-34` → `34`.
...
| `lib/form-payload.ts`                                               | `buildFormInsertPayload()` — `FormValues` → `FormInsertDto`      |
| `hooks/features/forms/use-create-form-record.ts`                    | `useCreateFormRecord()` — submit, snackbar, invalidación         |
| `hooks/queries/use-form-insert-mutation.ts`                         | `useFormInsert()` — mutación de inserción (`CREATE_FORM_ENTITY`) |
