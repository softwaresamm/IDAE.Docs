...
release_module: "Util - Descarga Masiva"
---

# Despliegue de Utilitario de Descarga Masiva en IIS

Esta guía detalla paso a paso cómo desplegar el Frontend (Next.js 15.3.2 + React 19 + TypeScript, `IDAE.UTIL.BulkOTSDownload`) en Windows Server usando IIS. Esta aplicación **no incluye backend .NET propio**: es un frontend Next.js standalone que consume la API SAMM ya desplegada, con autenticación vía NextAuth v4. IIS ejecuta el proceso Node.js a través de AspNetCore Module V2, usado aquí solo como gestor de procesos/reverse proxy — no porque la aplicación sea .NET.
...
| `-BasePath` | Sub-ruta pública de la app (se hornea en build) | `/react/descarga` |
| `-ApiUrl` | URL base de la API SAMM | `https://test.softwaresamm.com/sa_cliente/api` |
| `-NextAuthUrl` | URL pública completa **incluyendo el BasePath** | `https://test.softwaresamm.com/sa_cliente/react/descarga` |
| `-NextAuthSecret` | Secreto único de este despliegue para NextAuth | generar uno nuevo, no reutilizar |

:::important NextAuth
`NEXTAUTH_URL` debe ser la URL pública completa **incluyendo el BasePath**. Si se omite el BasePath, NextAuth construye las URLs de callback sin él y termina redirigiendo a `/api/auth/error` en vez de `/react/descarga/api/auth/error`. `NEXTAUTH_SECRET` debe ser único por cliente/despliegue — no reutilizar el mismo secreto entre clientes.
:::

:::warning Pendiente de confirmar con infraestructura
El propio script marca el `BasePath` (`/react/descarga`, dos segmentos) como pendiente de confirmar: revisar con Infraestructura si se despliega como una IIS Application anidada (`descarga` dentro de `react`) o de otra forma, antes del primer despliegue en un cliente nuevo.
:::
...
### Configuración de comportamiento propia de esta app (build-time)

A diferencia de `IDAE.UTIL.TechnicalReport` y `IDAE.UTIL.Certificates`, este proyecto tiene 6 variables adicionales `NEXT_PUBLIC_BULK_DOWNLOAD_*` que controlan qué reporte se descarga y con qué filtros. **Están hardcodeadas dentro del propio `build-for-iis.ps1`** (no son parámetros del script), así que un cliente que necesite otro reporte/filtro requiere editar el script antes de compilar, no solo pasar flags distintos:

| Variable | Valor por defecto en el script |
| --- | --- |
| `NEXT_PUBLIC_BULK_DOWNLOAD_DEFAULT_REPORT_CODE` | `formatos/pri_OT` |
| `NEXT_PUBLIC_BULK_DOWNLOAD_POLL_INTERVAL_MS` | `5000` |
| `NEXT_PUBLIC_BULK_DOWNLOAD_ESTADO_FILTER` | `/id_tipodocumento=2` |
| `NEXT_PUBLIC_BULK_DOWNLOAD_PARENT_DOCUMENT_ENTITY` | `documento.solicitud` |
| `NEXT_PUBLIC_BULK_DOWNLOAD_PARENT_DOCUMENT_FILTER` | (vacío en el script; el `.env.example` del repo trae un ejemplo con filtro) |
| `NEXT_PUBLIC_BULK_DOWNLOAD_PARENT_DOCUMENT_NAME` | `doc_documento_ot_id_documento` |

## Configuración

### Paso 1: Compilar el proyecto con `build-for-iis.ps1`

Ejecutar desde la raíz del repo `IDAE.UTIL.BulkOTSDownload` (donde está `package.json`):

```powershell title="Compilar para un cliente"
.\build-for-iis.ps1 -Client sa_cliente `
  -BasePath "/react/descarga" `
  -ApiUrl "https://test.softwaresamm.com/sa_cliente/api" `
  -NextAuthUrl "https://test.softwaresamm.com/sa_cliente/react/descarga" `
  -NextAuthSecret "<secreto-unico>"
```

El script instala dependencias (`npm install`), compila Next.js en modo standalone (`npm run build`) con las variables `NEXT_PUBLIC_BULK_DOWNLOAD_*` horneadas, copia `.next/standalone`, `.next/static` y `public/` a `%USERPROFILE%\Downloads\<Client>`, genera un `.env.production` de respaldo (con `API_URL`, `NEXT_PUBLIC_BASE_PATH`, `NEXTAUTH_URL`, `NEXTAUTH_SECRET`), y crea `iis-server.js` y `web.config`.

`BasePath` (y las variables `NEXT_PUBLIC_BULK_DOWNLOAD_*`) se hornean en tiempo de compilación, por lo que se compila una vez por cliente. `ApiUrl`/`NextAuthUrl`/`NextAuthSecret` en cambio quedan también en `web.config` y se leen en runtime — un cliente ya desplegado se puede repuntar (cambio de ambiente o de host de API) editando `web.config` en el servidor, sin recompilar.

### Paso 2: Copiar los archivos compilados al servidor

```powershell title="Copiar archivos al servidor"
New-Item -ItemType Directory -Path "C:\Samm\descarga" -Force
Copy-Item -Path "$env:USERPROFILE\Downloads\<Client>\*" -Destination "C:\Samm\descarga" -Recurse -Force
New-Item -ItemType Directory -Path "C:\Samm\descarga\logs" -Force
```

Ajuste la ruta física según la estructura del sitio/aplicación que confirme Infraestructura para el `BasePath` de dos segmentos (`/react/descarga`).

### Paso 3: Crear el Application Pool y el sitio/aplicación en IIS

```powershell title="Crear Application Pool"
Import-Module WebAdministration
New-WebAppPool -Name "descarga"
Set-ItemProperty -Path "IIS:\AppPools\descarga" -Name "managedRuntimeVersion" -Value ""
Set-ItemProperty -Path "IIS:\AppPools\descarga" -Name "processModel.identityType" -Value "ApplicationPoolIdentity"
Set-ItemProperty -Path "IIS:\AppPools\descarga" -Name "enable32BitAppOnWin64" -Value $false
Set-ItemProperty -Path "IIS:\AppPools\descarga" -Name "startMode" -Value "AlwaysRunning"
Restart-WebAppPool -Name "descarga"
```

```powershell title="Crear aplicación en IIS"
# Ajustar -Site/-PhysicalPath si el BasePath requiere una Application anidada
# (p. ej. "react/descarga" dentro de un sitio o aplicación "react")
New-WebApplication -Name "descarga" -Site "Default Web Site" -PhysicalPath "C:\Samm\descarga" -ApplicationPool "descarga"
Get-WebApplication -Name "descarga" -Site "Default Web Site"
```

```powershell title="Configurar permisos"
$acl = Get-Acl "C:\Samm\descarga"
$permission = "IIS AppPool\descarga", "ReadAndExecute", "ContainerInherit,ObjectInherit", "None", "Allow"
$accessRule = New-Object System.Security.AccessControl.FileSystemAccessRule $permission
$acl.SetAccessRule($accessRule)
Set-Acl "C:\Samm\descarga" $acl

$aclLogs = Get-Acl "C:\Samm\descarga\logs"
$permissionLogs = "IIS AppPool\descarga", "Modify", "ContainerInherit,ObjectInherit", "None", "Allow"
...
Set-Acl "C:\Samm\descarga\logs" $aclLogs
```
...
Confirme que ambos archivos existen en `C:\Samm\descarga\` antes de continuar.

### Paso 5: Reiniciar el Application Pool y probar

```powershell title="Reiniciar y probar"
Restart-WebAppPool -Name "descarga"
Start-Process "http://localhost/react/descarga"
Invoke-WebRequest -Uri "http://localhost/react/descarga" -UseBasicParsing
```
...
Set-WebConfigurationProperty -Filter /system.webServer/directoryBrowse `
  -Name enabled -Value $false -PSPath "IIS:\Sites\Default Web Site\descarga"

Set-WebConfigurationProperty -Filter /system.webServer/security/requestFiltering/requestLimits `
  -Name maxAllowedContentLength -Value 52428800 -PSPath "IIS:\Sites\Default Web Site\descarga"
...
Restart-WebAppPool -Name "descarga"
Stop-WebAppPool -Name "descarga"
Start-WebAppPool -Name "descarga"
...
1. **Frontend Web activo**: la aplicación estará disponible en `http://localhost/react/descarga` (o el `BasePath` configurado).
2. **Application Pool activo**: `descarga` en estado "Started", con `startMode` `AlwaysRunning`.
...
4. **Descarga masiva funcional**: el reporte configurado en `NEXT_PUBLIC_BULK_DOWNLOAD_DEFAULT_REPORT_CODE` se genera y descarga correctamente.
5. **Logs accesibles**: en `C:\Samm\descarga\logs\stdout_*.log` / `stderr_*.log`.
...
Confirme que `AspNetCoreModuleV2` está instalado, que Node.js está en `C:\Program Files\nodejs\node.exe`, y que `server.js`/`iis-server.js` existen en la carpeta. Revise `C:\Samm\descarga\logs\stdout_*.log`.
...
Confirme la versión de Node.js instalada, que `iis-server.js` existe con sintaxis correcta, y revise `C:\Samm\descarga\logs\stderr_*.log`.

### NextAuth redirige a `/api/auth/error` en vez de `/react/descarga/api/auth/error`

Verifique que `NEXTAUTH_URL` en `web.config` incluye el `BasePath` completo. Este es el error más común al desplegar apps con NextAuth sobre un sub-path de IIS.
...
### La descarga masiva trae el reporte o filtros equivocados

Revise las variables `NEXT_PUBLIC_BULK_DOWNLOAD_*` horneadas en el build de este cliente — son build-time, definidas dentro de `build-for-iis.ps1`; corregirlas requiere editar el script y recompilar, no solo tocar `web.config`.
